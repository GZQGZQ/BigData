...
string BigData::Add(string strLeft,string strRight)
{
	...
	string strTemp;
	strTemp.resize(LSize + 1);
	strTemp[0] = strLeft[0];

	char step = 0;
	for (size_t idx = 1; idx < LSize; ++idx)
	{
		char ret = strLeft[LSize - idx] + step - '0';
		if (idx < RSize)
			ret += strRight[RSize - idx] - '0';
		step = 0;
		if (ret >= 10)
		{
			step = 1;
